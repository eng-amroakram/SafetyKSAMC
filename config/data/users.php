<?php


return [
    "78896" => [
        "name" => "مدني عبدالله الهندي",
        "job_title" => "شركة سمامة",
        "id_number" => Null,
        "phone" => "",
        "email" => ""
    ],
    "096729" => [
        "name" => "حسين أبو عيفه",
        "job_title" => "شركة سمامة",
        "id_number" => Null,
        "phone" => "",
        "email" => ""
    ],
    "44277" => [
        "name" => "خالد مناور السحيمي",
        "job_title" => "عقود الحراسات",
        "id_number" => Null,
        "phone" => "",
        "email" => ""
    ],
    "44179" => [
        "name" => "حاتم سعود اللهيي",
        "job_title" => "عقود الحراسات",
        "id_number" => Null,
        "phone" => "",
        "email" => ""
    ],
    "44285" => [
        "name" => "حنان عبدالله الهندي",
        "job_title" => "عقود الحراسات",
        "id_number" => Null,
        "phone" => "",
        "email" => ""
    ],
    "44220" => [
        "name" => "امل حامد محمد الحجيلي",
        "job_title" => "شركة دار المعدات",
        "id_number" => "1070027238",
        "phone" => "",
        "email" => ""
    ],
    "51607694" => [
        "name" => "حنين عويض المطيري",
        "job_title" => "مسؤول امن صحي",
        "id_number" => "1080757063",
        "phone" => "",
        "email" => ""
    ],
    "44217" => [
        "name" => "بسام حمود المطيري",
        "job_title" => "شركة دار المعدات",
        "id_number" => "1097211971",
        "phone" => "",
        "email" => ""
    ],
    "4855366" => [
        "name" => "عاصم محمد عواد الأحمدي",
        "job_title" => "مسؤول الأمن الصحي",
        "id_number" => "1079076236",
        "phone" => "",
        "email" => ""
    ],
    "4855323" => [
        "name" => "إبراهيم عبدالحميد الصاعدي",
        "job_title" => "مسؤول الأمن الصحي",
        "id_number" => "1093329231",
        "phone" => "",
        "email" => ""
    ],
    "51607305" => [
        "name" => "نشمي عازم ناشي الحربي",
        "job_title" => "مسؤول الأمن الصحي",
        "id_number" => "1081313361",
        "phone" => "",
        "email" => ""
    ],
    "51605011" => [
        "name" => "عبدالباري نور السحيمي",
        "job_title" => "مسؤول الأمن الصحي",
        "id_number" => "1108714559",
        "phone" => "",
        "email" => ""
    ],
    "51605320" => [
        "name" => "محمد طلال عامر العوفي",
        "job_title" => "مسؤول الأمن الصحي",
        "id_number" => "1084592326",
        "phone" => "",
        "email" => ""
    ],
    "51607317" => [
        "name" => "عادل مساعد سعد الصاعدي",
        "job_title" => "مسؤول الأمن الصحي",
        "id_number" => "1090727288",
        "phone" => "",
        "email" => ""
    ],
    "4855987" => [
        "name" => "شهد عبيد الله البيضاني",
        "job_title" => "مسؤول الأمن الصحي",
        "id_number" => "1099446096",
        "phone" => "",
        "email" => ""
    ],
    "4855993" => [
        "name" => "احمد محمد عبيدالله السيد",
        "job_title" => "مسؤول",
        "id_number" => "1090787381",
        "phone" => "0556285944",
        "email" => "AhmAlsayed@moh.gov.sa"
    ],




    // "44190" => ["name" => "فوزي محمد المالكي", "job_title" => "عقود الحراسات", "id_number" => Null],
    // "44219" => ["name" => "إيمان محمد علي", "job_title" => "عقود الحراسات", "id_number" => Null],
    // "44218" => ["name" => "فاطمه عزيز الخناني", "job_title" => "عقود الحراسات", "id_number" => Null],
    // "12082" => ["name" => "أمل حمد الحجيلي", "job_title" => "عقود الحراسات", "id_number" => Null],
    // "4855745" => ["name" => "احمد معيض الكثيري", "job_title" => "مسؤول الأمن الصحي", "id_number" => "1093972477"],
    // "4855349" => ["name" => "ياسر سمير حويان الحربي", "job_title" => "مسؤول الأمن الصحي", "id_number" => "108451020"],
    // "4855349" => ["name" => "سعود عايض المطرفي", "job_title"=> "مسؤول الأمن الصحي", "id_number" => "1067874683"],
    // "4855358" => ["name" => "سلطان سلمان الحازمي", "job_title" => "مسؤول الأمن الصحي", "id_number" => "1083703023"],
    // "4855666" => ["name" => "المنصور محمد الحازمي", "job_title"=> "مسؤول الأمن الصحي", "id_number" => "1072254798"],
    // "4855649" => ["name" => "عيد سالم عياد الجهني", "job_title" => "مسؤول الأمن الصحي", "id_number" => "1077379426"],
    // "4855633" => ["name" => "أسامة سليمان العوفي", "job_title" => "مسؤول الأمن الصحي", "id_number" => "1112804891"],
    // "4856171" => ["name" => "عبير خليوي الحربي", "job_title" => "مسؤول الأمن الصحي", "id_number" => "1064792284"],
    // "4855645" => ["name" => "محمد صالح البركاني", "job_title" => "مسؤول الأمن الصحي", "id_number" => "1090101633"],
    // "4855688" => ["name" => "مهند سعد الحربي", "job_title" => "مسؤول الأمن الصحي", "id_number" => "1092518412"],
    // "4855701" => ["name" => "عبدالعزيز حميد الصاعدي", "job_title" => "مسؤول الأمن الصحي", "id_number" => "1015381443"],
    // "51605759" => ["name" => "عيسى محمد علي هزازي", "job_title" => "مسؤول الأمن الصحي", "id_number" => "1089928806"],
    // "51607311" => ["name" => "عبدالله هديبان عبدالله الصاعدي", "job_title" => "مسؤول الأمن الصحي", "id_number" => "1069990636"],
    // "51606245" => ["name" => "أسامة سليم حضيض الصاعدي", "job_title" => "مسؤول الأمن الصحي", "id_number" => "1099526814"],
    // "51605487" => ["name" => "عبدالله عايد عماش السهلي", "job_title" => "مسؤول الأمن الصحي", "id_number" => "1107876425"],
    // "4855644" => ["name" => "محمد حميد حامد العوفي", "job_title" => "مسؤول الأمن الصحي", "id_number" => "1112905029"],
];
